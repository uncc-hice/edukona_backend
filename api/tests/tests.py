...
class BaseTest(TestCase):
    ...
    def setUp(self):
        ...
        self.new_user_instructor = User.objects.create_user(username='test@gmail.com', password='password')
        ...
        self.new_user_student = User.objects.create_user(username='student@gmail.com', password='password')
        ...
        self.new_quiz_session = QuizSession.objects.create(
            quiz=self.new_quiz,
            code=QuizSession.generate_unique_code(self)
        )
...
class InstructorViewTest(BaseTest):
    ...
    def test_get_instructor(self):
        url = reverse('instructor-detail', kwargs={'instructor_id': self.new_user_instructor.instructor.id})
...
class StudentViewTest(BaseTest):
    ...
    def test_get_student(self):
        url = reverse('student-detail', kwargs={'student_id': self.new_user_student.student.id})
        response = self.client_student.get(url)

        self.assertEqual(response.status_code, 200)

        response_data = response.json()
        self.assertEqual(response_data['student']['id'], self.new_user_student.student.id)
...
class QuestionViewTest(BaseTest):
    def test_post_question(self):
        ...
        new_question = QuestionMultipleChoice.objects.get(id=response_data['question_id'])
        self.assertEqual(new_question.question_text, data['question_text'])
        self.assertEqual(new_question.incorrect_answer_list, data['incorrect_answer_list'])
    ...
    def test_get_question(self):
        ...
        self.assertEqual(response_data['questions']['question_text'], self.new_question.question_text)
        self.assertEqual(response_data['questions']['incorrect_answer_list'], self.new_question.incorrect_answer_list)
        self.assertEqual(response_data['questions']['correct_answer'], self.new_question.correct_answer)
    ...
    def test_get_all_questions(self):
        ...
        for question in questions:
            with self.subTest(question=question):
                ...
                self.assertEqual(question['incorrect_answer_list'], expected['incorrect_answer_list'])
...
class UserResponseViewTest(BaseTest):
    ...
    def test_put_user_response(self):
        ...
        data = {
            "student_id": self.new_quiz_session_student.id,
            "selected_answer": "1"
        }
...
class QuizSessionResultsTest(BaseTest):
    ...
    def test_quiz_session_results(self):
        ...
        response = self.client.get(reverse('quiz-session-results', kwargs={'code': self.new_quiz_session.code}))
...
class QuizSessionsByInstructorViewTest(TestCase):
    def setUp(self):
        ...
        self.instructor_user = User.objects.create(username='instructor@example.com', password='securepassword')
